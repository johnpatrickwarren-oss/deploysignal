...
**Scope.** ~~Docs-only this batch.~~ **Implemented 2026-07-17 (PR #50)**: direction classification (`engine/recalibration/classify.ts`), candidate state machine + persisted store with atomic promotion/rollback (`tools/recalibrate/`), operator-review CLI with reason codes and 14-day lazy timeout enforcement (no daemon — swept at CLI invocation), mandatory shadow validation via the existing shadow-compare machinery, the six `recalibration.*` lifecycle events, and integration tests covering the rejection-preserves-alarms invariant against the real engine. Remaining from this section's original list: maturity-dashboard event *consumers* (events are emitted; no dashboard), per-service reason-code catalog beyond the fixed seven codes, and automated candidate *production* (window selection → compile remains operator-driven; integrated refresh controller is approved follow-on).
