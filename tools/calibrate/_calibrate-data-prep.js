...
exports.choleskyLowerTriangularLocal = choleskyLowerTriangularLocal;
exports.parseArgs = parseArgs;
exports.loadBundle = loadBundle;
exports.flattenSignal = flattenSignal;
exports.quantile = quantile;
exports.collectCellSamples2D = collectCellSamples2D;
exports.collectFamilyCRows = collectFamilyCRows;
...
function choleskyLowerTriangularLocal(Sigma) {
...
function parseArgs(argv) {
    ...
    if (!args.baseline)
        throw new Error('--baseline <dir> required');
    if (!args.out)
        throw new Error('--out <file> required');
    ...
    if (familyESelectorRaw !== undefined) {
        if (familyESelectorRaw === 'auto' ||
            ...
            throw new Error(`--family_E_variant_selector must be one of auto | force_weighted | `
    ...
    return {
        ...
        cell_dimension_deficiency_mode: args.cell_dimension_deficiency_mode === 'warn'
            || args.cell_dimension_deficiency_mode === 'error'
            || args.cell_dimension_deficiency_mode === 'silent'
            ? args.cell_dimension_deficiency_mode
            : undefined,
...
function loadBundle(dir) {
    ...
    if (!fs.existsSync(manifestPath))
        throw new Error('manifest.json missing in ' + abs);
    if (!fs.existsSync(bundlePath))
        throw new Error('bundle.jsonl missing in ' + abs);
    ...
    if (manifest.tick_seconds !== undefined)
        bundle.tick_seconds = manifest.tick_seconds;
    if (manifest.baseline_provenance !== undefined)
        bundle.baseline_provenance = manifest.baseline_provenance;
    return bundle;
}
/** Flatten one signal across all runs × all ticks into a single array. */
function flattenSignal(bundle, signal) {
    const out = [];
...
function collectCellSamples2D(bundle, tenantTierMap) {
    ...
    const cellIndex = (h, d, tierPos) => tierPos * dayCount * 24 + (twoD ? d * 24 + h : h);
    for (const run of bundle.runs) {
        const hod = run.hour_of_day;
        if (!hod)
            throw new Error('Run missing hour_of_day[] — regenerate the baseline with a cell-aware generator.');
...
function collectFamilyCRows(bundle, tenantTierMap, jointVectorSignals = _calibrate_constants_js_1.FAMILY_C_SIGNALS) {
    const twoD = bundle.cell_dim === 'hour_of_day_x_day_of_week';
    const dayCount = twoD ? 7 : 1;
    const tiers = tenantTierMap ? _calibrate_constants_js_1.EMITTED_TIERS : ['aggregate'];
    const cells = [];
    ...
    const idx = (h, d, tierPos) => tierPos * dayCount * 24 + (twoD ? d * 24 + h : h);
