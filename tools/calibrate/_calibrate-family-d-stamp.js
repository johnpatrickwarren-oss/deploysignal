...
exports.attachFamilyDAndStamp = attachFamilyDAndStamp;
const ar1_js_1 = require("../../engine/resamplers/ar1.js");
const cholesky_js_1 = require("../../engine/resamplers/cholesky.js");
...
const _calibrate_audits_js_1 = require("./_calibrate-audits.js");
const PSD_EPS_RELATIVE = 1e-9;
const ALPHA_C_FAMILY = 2e-4; // Family C α budget (matches runtime default)
const cellSeedOffset = (key) => ((Number(key.hour_of_day) * 31 + Number(key.day_of_week)) * 7
...
function collectPerTierSignal(bundle, tenantTierMap) {
    const perTierSignal = {
        dominant: {}, large: {}, medium: {}, small: {}, aggregate: {},
    };
    for (const run of bundle.runs) {
        const runTier = tenantTierMap && run.tenant_id
        ...
        for (const sig of Object.keys(run.signal_series)) {
            ...
            for (const v of run.signal_series[sig])
                aggBucket.push(v);
...
function fitPerTierFamilyD(perTierSignal, alphaDBoot, useLegacyD) {
    const TIER_SEED_SALT = {
    ...
    const perTierFamilyD = {
        dominant: {}, large: {}, medium: {}, small: {}, aggregate: {},
    };
    for (const tier of _calibrate_constants_js_1.EMITTED_TIERS) {
        const samplesByTier = perTierSignal[tier];
        for (const sig of Object.keys(samplesByTier)) {
            const seed = (family_d_js_1.FAMILY_D_BOOTSTRAP_SEED + sig.length) ^ TIER_SEED_SALT[tier];
            const { result } = (0, family_d_js_1.buildFamilyDForSignalAR1)(samplesByTier[sig], alphaDBoot, seed, useLegacyD);
            if (result)
                perTierFamilyD[tier][sig] = result;
        }
    }
...
function stampJointAR1CholeskyAll(baselineCells, perTierFamilyD, familyCSignals) {
    const aggFamilyD = perTierFamilyD.aggregate;
    const stampJointAR1Cholesky = (cellFamilyC, tier) => {
        if (!cellFamilyC.covariance)
            return;
        if (cellFamilyC.covariance.length !== familyCSignals.length)
            return;
        const tierFD = perTierFamilyD[tier];
        const rhoVec = familyCSignals.map((sig) => tierFD[sig]?.ar1_phi ?? aggFamilyD[sig]?.ar1_phi ?? 0);
        const sigmaEpsRaw = (0, ar1_js_1.computeWhiteNoiseCovariance)(cellFamilyC.covariance, rhoVec);
        const sigmaEpsPSD = (0, _shared_js_1.projectPSD)(sigmaEpsRaw, PSD_EPS_RELATIVE);
        cellFamilyC.cholesky_L_eps = (0, cholesky_js_1.cholesky)(sigmaEpsPSD);
    };
...
function hotellingCacheKey(sigma, rho, alphaArg, variantArg, safeParams) {
    const round = (v) => Number.isFinite(v)
        ? v.toExponential(12) : String(v);
    const parts = [variantArg, round(alphaArg)];
    for (const r of rho)
        parts.push(round(r));
...
function stampSlidingBufferHotellingAll(baselineCells, perTierFamilyD, familyCSignals) {
    const aggFamilyD = perTierFamilyD.aggregate;
    const slidingBufferCache = new Map();
    const stampSlidingBufferHotellingThreshold = (cellFamilyC, tier, seed) => {
        if (!cellFamilyC.covariance || !cellFamilyC.cholesky_L_eps)
            return;
        if (cellFamilyC.covariance.length !== familyCSignals.length)
            return;
        ...
        const sigmaEpsRaw = (0, ar1_js_1.computeWhiteNoiseCovariance)(cellFamilyC.covariance, rhoVec);
        const sigmaEpsPSD = (0, _shared_js_1.projectPSD)(sigmaEpsRaw, PSD_EPS_RELATIVE);
        const alphaForBootstrap = variant === 'safe_test' && safeParams ? safeParams.alpha : alphaHotelling;
        const key = hotellingCacheKey(cellFamilyC.covariance, rhoVec, alphaForBootstrap, variant, safeParams);
        let result = slidingBufferCache.get(key);
        if (!result) {
            result = (0, family_c_js_1.bootstrapHotellingSlidingBufferThreshold)(cellFamilyC.mean_vector, cellFamilyC.covariance, rhoVec, sigmaEpsPSD, alphaForBootstrap, variant, safeParams, family_c_js_1.FAMILY_C_HOTELLING_BOOTSTRAP_SEED + seed);
    ...
    for (const entry of baselineCells.cells) {
        if (entry.family_C) {
            const tier = entry.key.tenant_tier ?? 'aggregate';
            stampSlidingBufferHotellingThreshold(entry.family_C, tier, cellSeedOffset(entry.key));
        }
    }
    if (baselineCells.aggregate_fallback.family_C) {
        stampSlidingBufferHotellingThreshold(baselineCells.aggregate_fallback.family_C, 'aggregate', 0);
    }
}
...
function stampBettingSlidingBufferAll(baselineCells, perTierFamilyD, effective, alphaTotal, familyASignalsLength) {
    ...
    const stampBettingThreshold = (perSignal, tier, seedOffset) => {
        if (!perSignal)
            return;
        const tierFD = perTierFamilyD[tier];
        for (const sig of Object.keys(perSignal)) {
            ...
            if (!res) {
                res = (0, family_a_js_1.bootstrapBettingSlidingBufferThreshold)(mu, sigma2, rho, alphaBettingPerSignalLocal, family_a_js_1.FAMILY_A_BETTING_BOOTSTRAP_SEED + seedOffset);
                cache.set(key, res);
            }
    ...
    if (baselineCells.aggregate_fallback.family_A?.per_signal) {
        stampBettingThreshold(baselineCells.aggregate_fallback.family_A.per_signal, 'aggregate', 0);
    }
}
/** Stamp per-cell family_D using the cell's tier ρ (forward-compat). */
function stampPerCellFamilyD(baselineCells, perTierFamilyD) {
    for (const entry of baselineCells.cells) {
        const tier = entry.key.tenant_tier;
...
function attachFamilyDAndStamp(bundle, baselineCells, tenantTierMap, compilerOpts, effective, alphaTotal, familyDAlphaFraction, familyCSignals, familyASignalsLength) {
    ...
    (0, _calibrate_audits_js_1.auditAR1FactorizationConsistency)(baselineCells, perTierFamilyD, familyCSignals);
    // Q2.B.6.2 — sliding-buffer Hotelling threshold consistency audit.
    (0, _calibrate_audits_js_1.auditSlidingBufferHotellingConsistency)(baselineCells, familyCSignals);
    // Q2.B.6.3 — Family A betting sliding-buffer recalibration.
    stampBettingSlidingBufferAll(baselineCells, perTierFamilyD, effective, alphaTotal, familyASignalsLength);
    // Q2.B.6.3 — sliding-buffer betting threshold consistency audit.
    (0, _calibrate_audits_js_1.auditBettingSlidingBufferConsistency)(baselineCells);
}
