...
- **Architectural placement:** Cairn ships as a sibling product at https://github.com/johnpatrickwarren-oss/cairn (originally landed inside DS at `engine/cairn/` via DS PR #21; extracted post-merge for architectural consistency with the rest of the bundle). Consumes existing wire shapes: DS audit JSONL, Tessera VerdictGroupPayload, Anvil ExpectedFailurePattern, and a generic external-event JSON shape (incident-mgmt webhook payloads, env-change feeds). No new detector family on the DS side — Q2.B.6.4 ADR preserved.
- **Scope:** Runway-substantive — Cairn has real attribution math (Gaussian timestamp-alignment kernel × per-kind prior × evidence-quality boost; mechanistic-inconsistency suppression; engine-inferred onset preference), not a stub-only positioning play.
- **Effort:** Medium (typed contracts + scoring algorithm + 4 ingest helpers + CLI + 23 tests + walkthrough doc + bundle positioning across DS / Tessera / Anvil documentation).
