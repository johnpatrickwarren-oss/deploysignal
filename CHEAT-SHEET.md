...
**Orchestration adapters [partially shipped, 2026-07-17]:** the **Argo Rollouts Level-1 web-provider adapter is shipped as a runnable service** (`service/gate-http/` — `GET /v1/verdict/{deploy-ref}` per `ORCHESTRATION-ADAPTERS.md`'s spec-verbatim response shape, push tick ingestion, durable sessions with declare-void-and-restart semantics, fail_closed default, shadow mode, AnalysisTemplate YAML with a contract test). Spinnaker / model-lifecycle / webhook adapters remain **[spec-only]**; the Anvil chaos family (`engine/o0/anvil/`) remains typed contracts with deliberately-throwing network stubs. (An earlier revision of this line listed all adapters as shipped, then none; both were wrong in their day — this is the current truth.)

**Shipped 2026-07-17 [offline-tool]:** direction-aware baseline-maintenance lifecycle (North-Star Addition #15) — `tools/recalibrate` with persisted candidates, readiness gates incl. operator-declared exclusion windows, mandatory shadow validation, improvement auto-promote / degradation review with reason codes, 14-day timeout default-reject, calendar due-check, atomic promotion + rollback. Candidate *production* (window selection → curation → compile) remains upstream; an integrated refresh controller is approved follow-on.

**Specified, implementation-for follow-on [spec-only]:** real-production-baseline calibration (synthetic baseline today; needs >100K healthy prod runs for tight α calibration); live cluster integration against a running Argo deployment; Tier-2 neural detectors (foundation-model novelty, transformer CPD) gated on real-baseline accumulation; SLO substrate derivation from baselines.
...
**Deferred (for follow-on):** runs of the shadow-compare CLI against the 5 profiles (the CLI itself, `tools/run-shadow-compare.ts`, is shipped [offline-tool]; the synthetic-vs-real-baseline comparison runs are not), audit-emitted report cards per profile, NAB firewall for Families A/D structural-validity floor (first committed runs live in deploysignal-engine's `validation/nab/` (2026-07-17) and are BELOW the Q64.2 floors — A best 35.50 < 50, D 29.79 < 40 — the firewall must not be cited as passed), v9b/v9c ingestion (AlpaServe, DeepSpeed-FastGen — not yet acquired).
...
- **Ville-bounded FPR — DUAL-SURFACE (post-REPLY-52gk):**
  ...
  _Superseded in part at Q2.B.6._ The figures above are the pre-Q2.B.6 record, kept as what was believed at REPLY-52gk. The enforcement landed: parametric Cholesky family_C fires 131→0; iid bootstrap family_C fires 131→6. **Family D stays open** — 21/131 residual Ville-bounded fires under parametric H₀, all `family_D_kv_cache`, deferred to the Q2.B.5b investigation (see `NORTH-STAR-ARCHITECTURE.md` §Q2.B.6).
