...
- **PRD references unverified codebase entities.** PRD scope blocks that name specific schema fields (`user.tenantId`, `project.completedAt`), function names, file paths, or environment variables MUST be grep-verified against the current codebase before launching a round. A reference assumed from memory or from a prior version of the schema — e.g., a field that was renamed, deferred, or never landed — silently forces a downstream pipeline halt: the role-pipeline catches it via DIAGNOSTIC + ESCALATE, but the operator-side cost is the round's elapsed time before the halt. The same static-analysis discipline the Implementer applies in the spec ("when applying X to all instances of Y, enumerate via grep — not memory") applies upstream to the PM: for every codebase reference in the PRD, run `grep -r '<entity>' <relevant-source-or-schema>` and confirm before routing. The role-pipeline is the safety net; this discipline is the prevention.
