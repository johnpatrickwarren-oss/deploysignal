...
### Addition #15 — baseline-maintenance candidate lifecycle (shipped 2026-07-17)

Per NORTH-STAR-ARCHITECTURE.md § Addition #15 and the implementation plan's §A7/OQ-8 disposition. A drift/calendar/operator trigger proposes a CANDIDATE compiled baseline that moves through readiness gates, shadow validation, and (operator or auto-) approval before it can ever become the active baseline — see `engine/types/recalibration.ts` for the full candidate contract and `engine/recalibration/state-machine.ts` for the pure lifecycle transition function.

- **`AuditRecordV2.recalibration_event`** (optional, type-only): `{candidate_id, outcome, operator_id?, reason_code?}` — `RecalibrationEventRecord` (`engine/types/recalibration.ts`), the same shape persisted to the candidate store's `events.jsonl`. **Type-only per OQ-8: no writer-side commitment.** Neither `AuditWriter` nor `buildAuditRecord` populate this field in this project — it exists so a per-tick audit record COULD carry its triggering/resulting recalibration decision inline once a writer lands, without a schema-version bump (strict-additive, matches the Addition #27 `agent_proposal_emitted` / `agent_proposal_downgraded` top-level-audit-event precedent for "engine emits the shape, orchestrator wiring is a later increment").
- **Authoritative decision log stays `events.jsonl`.** The candidate-lifecycle store (`runs/baseline-history/<service_id>/events.jsonl`, one `RecalibrationEventRecord`-shaped line per lifecycle decision) is the system of record for "what happened to this candidate and why" — not the audit log. `recalibration_event` on an `AuditRecordV2`, if a future writer populates it, is a convenience join key for tooling that already streams audit records; it is never the primary source consumers should reconcile against.
- **`recalibration.*` lifecycle events** (six types, `engine/o0/lifecycle-events.ts`): `recalibration.proposed | .shadow_validated | .operator_approved | .operator_rejected | .auto_promoted | .timeout_rejected`. Strict-additive alongside the existing `evaluation.*` / `verdict_group.*` / `agent_proposal.*` types; ship the same `LifecycleEventEmitter` contract (`NoOpLifecycleEventEmitter` / `InMemoryLifecycleEventEmitter`) as Addition #14. Base payload on every type: `service_id, candidate_id, proposed_baseline_version, current_baseline_version, direction_classification, at`. Extras: `operator_id` + `reason_code` on `.operator_approved` / `.operator_rejected`; `shadow_report_path` on `.shadow_validated`; an `escalation: {escalated, escalated_to}` object embedded in `.timeout_rejected` (OQ-6 — the lazy-timeout sweep's default-reject-and-escalate is one event, not two: escalation is a fact ABOUT the timeout rejection, not a separate occurrence). Like `evaluation.*`, these are a derivative ephemeral stream, not audit records — the engine emits them; the candidate-lifecycle store's `events.jsonl` (not the lifecycle-event stream) is what persists.
