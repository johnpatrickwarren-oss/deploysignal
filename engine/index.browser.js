...
function evaluateState(_id, _cloud, ctx) {
    if (!ctx)
...
function progressScaleFor(v) {
    if (v.signal && WEALTH_SIGNAL_NAMES.has(v.signal))
        return 'wealth';
    if (WEALTH_REASON_CODES.has(v.reason_code))
        return 'wealth';
...
function maxProgressOfScale(vs, scale) {
    let best = null;
    for (const v of vs) {
        if (progressScaleFor(v) !== scale)
            continue;
...
function pickScaleAndProgress(vs) {
    const wealth = maxProgressOfScale(vs, 'wealth');
    if (wealth !== null)
        return { progress: wealth, scale: 'wealth' };
...
function summarizeSingleCDetector(v, label, kind) {
    const state = v.verdict === 'fire' ? 'fired'
...
function summarizeFamilyC(famC, famCMmd) {
    ...
    const out = [];
    if (famC)
        out.push(summarizeSingleCDetector(famC, 'Hotelling T²', 'hotelling'));
    if (famCMmd)
        out.push(summarizeSingleCDetector(famCMmd, 'Sequential MMD', 'e_mmd_betting'));
    return out;
}
 ...
 *  process. */
function renderAccumulatingNote(r) {
    const prefix = notePrefix(r);
    if (r.progress === null)
        return `${prefix} accumulating evidence`;
...
function renderNote(r) {
    ...
    if (r.state === 'accumulating')
        return renderAccumulatingNote(r);
