...
(function () {
  ...
  var { FAMILY_C_SIGNALS, lookupFamilyCParams } = __NS__;
  var { trafficGateMin } = __NS__;
  var { rbf, generateBaselinePool, baselinePoolSeed, BASELINE_POOL_SIZE } = __NS__;
  var { q72TraceEnabled, q72EmitProcessHeader, q72EmitCellHeader, q72EmitTick } = __NS__;
  var { computeRffFeatureMap, applyRffFeatureMap, RFF_DEFAULT_DIM } = __NS__;
...
function evaluateFamilyCBettingEProcess(cfg, liveMetrics, states, ctx) {
    ...
    if (!bp)
        return null; // cell not compiled with Q67 params (pre-Phase-3.d.B)
    ...
    if (ctx.schemaContinuityClass && shouldSuppress(ctx.schemaContinuityClass, 'C')) {
        return suppressedVerdict(ctx.schemaContinuityClass === 'observability_stack'
    ...
    if (ctx.ticksSinceDeploy < bake.min_ticks)
        return suppressedVerdict('bake_profile_not_met', threshold, null);
    if (ctx.deployAgeDays > bake.max_days)
        return suppressedVerdict('bake_profile_not_met', threshold, null);
    if (ctx.trafficPct < trafficGateMin(cfg))
        return suppressedVerdict('traffic_pct_below_gate', threshold, null);
    // Project live metrics into the Family C relative-deviation vector.
    ...
    if (!rffActive && (!pool || pool.length === 0)) {
        ...
        pool = generateBaselinePool(params, poolSize, baselinePoolSeed({ hour_of_day: ctx.hourOfDay, day_of_week: ctx.dayOfWeek }));
        states[poolKey] = pool;
    }
    ...
    if (rffActive && fm && bp.baseline_rff_mean && state.q_running_phi_sum) {
        ...
        F_t = computeKernelMMDWitness(v, pool ?? [], state.q_running_sum, state.q_count, bp.kernel_bandwidth_sigma, state.witness_running_max, state.n);
    }
    ...
    if (familyCEnabled) {
        ...
        if (tb) {
            ...
            try {
                ...
                const fcb = evaluateFamilyCBettingEProcess(opts.compiledConfig, liveMetrics, fcbStates, {
                    hourOfDay: opts.currentHourOfDay ?? new Date().getHours(),
                    dayOfWeek: opts.currentDayOfWeek,
                    ticksSinceDeploy: opts.ticksSinceDeploy ?? 0,
                    deployAgeDays: opts.deployAgeDays ?? 0,
...
function buildFamilyVerdictsV2(params, result, hr) {
    const empty = () => ({ verdict: 'clean', detectors: [], alpha_spent: 0, suppression_reason: null });
    const out = {
        ...
        for (const { v, rid, label } of familyCVerdicts) {
            if (v.verdict === 'fire') {
                ...
                if (sig === 'hotelling_t2_safe' && params.compiledConfig?.baseline_cells
                    ...
                    if (shrink !== undefined)
                        provExtras = { family_c_shrink_fraction_used: shrink };
                }
...
function maybeIngestAndFanOut(params, deployId, lifecycleEmitter, reversibility, fused, tick, total) {
    ...
    const topoLeg = params.topologyEnricher
